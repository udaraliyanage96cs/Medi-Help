                ...
                <div class="row">
                                        ...
                                        @foreach($reports as $report)
                                            <li class="list-group-item border-0 d-flex justify-content-between ps-0 mb-2 border-radius-lg">
                                                <div class="d-flex flex-column">
                                                    <a href="/uploads/reports/{{$report->patient_id}}/{{$report->url}}">
                                                ...
                                                <div class="d-flex align-items-center text-sm">
                                                    ...
                                                    <a data-toggle="tooltip" data-placement="top" title="Copy To Clipboard" onclick="copyToClipboard('{{URL::to('/')}}/uploads/reports/{{$report->patient_id}}/{{$report->url}}')" class="btn btn-link text-dark text-sm mb-0 px-0 ms-4" target="_blank" >
                    ...
                    <div class="col-12 col-xl-4 ml-auto">
                        <div class="card card-plain h-100">
                            ...
                            <div class="card-body p-3">
                                @if($reciptCH) 
                                    <form action="/{{Auth()->user()->role}}/channels/recipts/edit/{{$reciptCH->id}}" method="post">
                                        {{ csrf_field() }}
                                        <div class="form-group">
                                            <label for="">Reason
                                                <span class="red_star">*</span></label>
                                                <input type="text" name="reason" id="reason" class="form-control" required value=" {{$reciptCH->reason}}"/>
                                            </div>
                                        <div class="form-group">
                                            <label for="">Content <span class="red_star">*</span></label>
                                            <textarea name="editor1" required>{{$reciptCH->content}}</textarea>
                                        </div>
                                        <div class="form-group">
                                            <input type="submit" name="submit" id="submit" class="btn bg-gradient-primary" value="Update" />
                                        </div>
                                    </form>
                                @else
                                    <form action="/{{Auth()->user()->role}}/channels/recipts/add/{{$ticketID}}" method="post">
                                        {{ csrf_field() }}
                                        <div class="form-group">
                                            <label for="">Reason
                                                    <span class="red_star">*</span></label>
